...
class Weighted_Cross_Entropy_Loss(torch.nn.Module):
    ...
    def forward(self, pred, target, weights=torch.tensor([1,15]).to('cuda')):
        ...
        weighted_logp = (logp * weights.view(1, -1, 1, 1, 1))
...
"""
def binary_iou_loss_3d(pred, target):
    Iand = torch.sum(pred * target, dim=(1, 2, 3, 4))
    Ior = torch.sum(pred, dim=(1, 2, 3, 4)) + torch.sum(target, dim=(1, 2, 3, 4)) - Iand
    IoU = 1 - Iand / Ior
    return IoU.mean()
...
class IoULoss3D(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        for i in range(num_classes):
            loss = binary_iou_loss_3d(pred[:, i], target[:, i])
            total_loss += loss
...
class U3PLloss(nn.Module):
    def __init__(self, loss_type='focal', aux_weight=0.4, process_input=True):
        ...
        if loss_type == 'u3p':
            self.iou_loss = IoULoss3D(process_input=not process_input)
            self.ms_ssim_loss = MS_SSIMLoss3D(process_input=not process_input)
    ...
    def _forward_focal(self, preds, targets):
        ...
        loss_dict['head_focal_loss'] = loss.detach().item()
    ...
    def onehot_softmax(self, pred, target, process_target=True):
        _, num_classes, d, h, w = pred.shape
        pred = F.softmax(pred, dim=1)

        if process_target:
            target = torch.clamp(target, 0, num_classes)
            target = F.one_hot(target, num_classes=num_classes+1)[..., :num_classes].permute(0, 4, 1, 2, 3).contiguous().to(pred.dtype)
        return pred, target
...
class SSIMLoss(nn.Module):
    def __init__(self, win_size: int = 11, nonnegative: bool = True, process_input: bool = True):
        ...
        self.kernel = gaussian_kernel3d(win_size, 1)
    ...
    def forward(self, pred, target):
        _, num_classes, d, h, w = pred.shape
        win_size = min(d, h, w, self.win_size)
        kernel = self.kernel if win_size == self.win_size else gaussian_kernel3d(win_size, 1)
        kernel = kernel.to(pred.dtype).to(pred.device)

        if self.process_input:
            pred = F.softmax(pred, dim=1)
            target = F.one_hot(target, num_classes=num_classes).permute(0, 2, 1, 3, 4).float()
...
class MS_SSIMLoss3D(nn.Module):
    def __init__(self,
        ...
        super(MS_SSIMLoss3D, self).__init__()
        self.kernel = gaussian_kernel3d(win_size, 1)
...
def ms_ssim(x: Tensor,
    ...
    for i in range(m):
        if i > 0:
            x = F.avg_pool3d(x, kernel_size=2, ceil_mode=True)
            y = F.avg_pool3d(y, kernel_size=2, ceil_mode=True)
            d, h, w = x.shape[-3:]
...
def ssim_index(img1: Tensor,
    ...
    _, channel, depth, height, width = img1.shape
    ...
    mean1 = F.conv3d(img1, kernel, padding=p, groups=channel, stride=s)
    mean2 = F.conv3d(img2, kernel, padding=p, groups=channel, stride=s)
    mean12 = mean1 * mean2
    mean1 = mean1.pow(2)
    mean2 = mean2.pow(2)

    var1 = F.conv3d(img1 ** 2, kernel, padding=p, groups=channel, stride=s) - mean1
    var2 = F.conv3d(img2 ** 2, kernel, padding=p, groups=channel, stride=s) - mean2

    covar = F.conv3d(img1 * img2, kernel, padding=p, groups=channel, stride=s) - mean12
    ...
    if channel_avg:
        ...
        ss, cs = ss.flatten(3), cs.flatten(3)
...
def gaussian_kernel3d(kernel_size: int, channel: int = 1):
    k = gaussian_kernel(kernel_size, 1.5)
    k = torch.einsum('i,j->ij', [k, k])
    k3d = torch.einsum('i,j,k->ijk', [k, k, k])
    return k3d.expand(channel, 1, kernel_size, kernel_size, kernel_size).contiguous()
